...
aes = {
    "sgd": {"color": "#7fb285", "markevery": 15, "zorder": 7},
    ...
    "sgd-m, init_name=teleport": {
        "color": "#de9151",
        "markevery": 8,
        "zorder": 8,
    },
    "sgd-m, init_name=Default": {
        "color": "#7fb285",
        "markevery": 8,
        "zorder": 8,
    },
    "sps, init_name=teleport": {
        "color": "#F7B801",
        "markevery": 8,
        "zorder": 8,
    },
    "sps, init_name=Default": {
        "color": "#648381",
        "markevery": 8,
        "zorder": 8,
    },
    "momo, init_name=teleport": {
        "color": "#023047",
        "markevery": 8,
        "zorder": 8,
    },
    "momo, init_name=Default": {
        "color": "#a832a0",
        "markevery": 8,
        "zorder": 8,
    },
...
class Record:
    def __init__(
        self, exp_id: Union[str, list], output_dir="output/", as_json=True
    ):
        self.exp_id = exp_id
        ...
        if isinstance(exp_id, str):
            ...
            warnings.warn(
                "Loading from multiple output files. Contents will be merged."
            )
    ...
    def _build_raw_df(self):
        ...
        for r in self.data:
            ...
            init_dict = copy.deepcopy(r["config"]["init"])
            init_dict = {
                "init_name": init_dict.pop("name", ""),
                **init_dict,
            }  # move name to beginning
            ...
            id.append("dataset=" + r["config"]["dataset"])
            id.append("reg=" + str(r["config"]["reg"]))

            this_df["id"] = ":".join(
                id
            )  # identifier given by all opt specifications
            this_df["run_id"] = r["config"][
                "run_id"
            ]  # differentiating multiple runs
        ...
        try:
            df = df.drop(columns="step_size_list")
        except:
            pass
        ...
        if df.duplicated(subset=["id", "epoch", "run_id"]).any():
            raise KeyError(
                "There seem to be duplicates (by id, epoch, run_id). Please check the output data."
            )
    ...
    def _build_base_df(self, agg="mean"):
        ...
        if agg in ["mean", "median"]:
            df = (
                raw_df.groupby(["id", "epoch"], sort=False)
                .mean()
                .drop("run_id", axis=1)
            )
            df2 = (
        ...
        df = df.merge(
            self.id_df, how="left", left_on="id", right_index=True
        )  # add columns from id_df
    ...
    def plot_metric(
        ...
        for i, m in enumerate(df.id.unique()):
            this_df = df[df.id == m]
            x = this_df.loc[:, "epoch"]
            y = this_df.loc[:, s]
            ...
            label = conf["name"] + "_" + conf["lr"] + "_" + conf["init_name"]
            # plot
            if not y.isna().all():
                ...
                ax.plot(
                    ...
                    alpha=alpha,
                    label=label,
                    zorder=self.aes.get(conf["name"], self.aes["default"]).get(
                        "zorder"
                    ),
                )
...
def id_to_dict(id, add_underscore=False):
    ...
    if add_underscore:
        tmp = [
            "_" + t for t in tmp
        ]  # let each column start with _ to indicate that it is added afterwards
