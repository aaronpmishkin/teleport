...
def filter_result(exp_metrics, exp_config, logreg=True, batch_size=64):
...
exp_list = configs.expand_config_list(EXPERIMENTS["figures_9_10"])
...
for logreg in [True, False]:
    ...
    (
        ...
        problem_key,
        method_key,
        partial(filter_result, logreg=logreg, batch_size=64),
        best_obj,
    )
    ...
    (
        ...
        problem_key,
        method_key,
        partial(filter_result, logreg=logreg, batch_size="full_batch"),
        best_obj,
    )
    ...
    if logreg:
        figure_name = f"figure_9"
    else:
        figure_name = f"figure_10"

    os.makedirs("figures/", exist_ok=True)
    plt.savefig(f"figures/{figure_name}_init.pdf")
